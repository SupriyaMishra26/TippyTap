...
// import StepFirst from '../../assets/s1.jpeg';

// import StepSecond from '../../assets/s2.jpeg';
// import StepThird from '../../assets/s3.jpeg';
// import StepFourth from '../../assets/s4.jpeg';
...
//     description: 'Clean each nail with the alcohol pad to remove oils. Gently push back cuticles and lightly buff the surface for better grip.',
//     image: StepFirst,
//   },
//   {
//     number: '02',
//     title: 'Nail Size Matching',
//     description: 'Match each press-on to your natural nail so it fully covers from edge to edge. Lay them out in order before applying',
//     image: StepSecond,
//   },
//   {
//     number: '03',
//     title: 'Apply Glue',
//     description: 'Place a small dot of glue on your natural nail or the press-on. You can also use glue tabs for a quick, mess-free option.',
//     image: StepThird,
//   },
//   {
//     number: '04',
//     title: 'Press & Secure',
//     description: 'Press firmly for about 30 seconds, starting from the cuticle and moving outward. File edges if needed for a smooth finish.',
//     image: StepFourth,
//   },
// ];
...
//     location: 'Mumbai',
//     quote:
//       'Thank you, Jasmeen, for these absolutely beautiful nails. They are truly the best I’ve ever had. I’ve gotten extensions from other places before, but these are by far the best.  The detailing is exquisite, and the overall finish is flawless. Perfect Valentine’s nails. Thank you so much❤️❤️',
//   },
//   {
//     initials: 'IB',
//     name: 'Ipsita Bagchi',
//     location: 'Delhi',
//     quote:
//       'The best studio in Noida for your nails! Their work is top notch, & the service is definitely praiseworthy! Jasmeen is the sweetest and will guide you through the entire process! Quick, efficient and beautiful❤️ Thank you for a great experience!',
//   },
//   {
//     initials: 'DM',
//     name: 'De Mona Phijam',
//     location: 'Bengaluru',
//     quote:
//       'I had an amazing experience with Shivani and Archana .They are super talented, detail-oriented, and really takes her time to make sure every nail looks perfect. The salon was clean and welcoming, and my nails turned out exactly how I wanted. Highly recommend!',
//   },
// ];
...
function CollectionCard({ item }: { item: CollectionLook }) {
  ...
  const id = `home-${item.name.toLowerCase().replace(/\s+/g, '-')}`;
  ...
  return (
    <motion.article
      whileHover={{ y: -6 }}
      transition={{ duration: 0.25, ease: 'easeOut' }}
      className="group flex flex-col overflow-hidden rounded-[2rem] border border-[#edd5dd] bg-white shadow-[0_24px_60px_rgba(115,28,52,0.12)]"
    >
      <div className="relative aspect-[4/5] overflow-hidden bg-[#fff0f5]">
        <img
          src={item.image}
        ...
        <button
          type="button"
          onClick={() => {
            toggleWishlist({ id, name: item.name, price: priceNum, image: item.image });
            toast.success(liked ? 'Removed from wishlist' : 'Added to wishlist');
          }}
          className="absolute right-4 top-4 inline-flex h-9 w-9 items-center justify-center rounded-full bg-white/95 text-[#cb1f52] shadow-md transition hover:scale-110"
          aria-label="Toggle wishlist"
        >
          <Heart className={`h-4 w-4 ${liked ? 'fill-[#cb1f52]' : ''}`} />
        </button>
      </div>

      <div className="flex flex-1 flex-col border-t border-[#f3e1e8] bg-white p-5">
        <div className="flex items-start justify-between gap-4">
          <div>
            <h3 className="text-lg font-semibold text-[#2b1020] sm:text-xl">{item.name}</h3>
            <p className="mt-2 text-sm leading-6 text-[#6d4b58]">{item.description}</p>
        ...
        <div className="mt-4 flex gap-2">
          <button
            type="button"
            onClick={() => {
              addToCart({ id, name: item.name, price: priceNum, image: item.image });
              toast.success('Added to cart');
            }}
            className="inline-flex flex-1 items-center justify-center gap-2 rounded-full bg-[#cb1f52] px-4 py-2.5 text-sm font-semibold text-white shadow-[0_10px_24px_rgba(203,31,82,0.24)] transition hover:-translate-y-0.5 hover:bg-[#b71848]"
          >
            <ShoppingBagIcon /> Add to Cart
          </button>
          <Link
            to="/products"
            className="inline-flex items-center justify-center rounded-full border border-[#e7c4cf] bg-white px-4 py-2.5 text-sm font-semibold text-[#7b1730] transition hover:-translate-y-0.5 hover:border-[#cb1f52]"
          >
            View
          </Link>
        </div>
      ...
      >
        ...
        <div className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          <div className="mt-10 flex justify-center">
            <Link
              to="/products"
              className="inline-flex items-center gap-2 rounded-full bg-[#cb1f52] px-7 py-3 text-sm font-semibold text-white shadow-[0_18px_36px_rgba(203,31,82,0.22)] transition hover:-translate-y-0.5 hover:bg-[#b71848]"
            >
              View All Products
              <ArrowRight className="h-4 w-4" />
            </Link>
